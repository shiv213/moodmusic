...
def upload_file():
    ...
    if file:
        # You can save the file to the filesystem
        # or pass it directly to your processing function
        ...
        if file.filename.split(".")[-1] != "mp3":
            ...
            audio.export(os.path.join(app.config['UPLOAD_FOLDER'], file.filename + ".mp3"), format = "mp3")
            process_file(os.path.join(app.config['UPLOAD_FOLDER'], file.filename + ".mp3"))
            os.remove(filepath)


        # Here, you can call your Python script or function to process the uploaded file
        # e.g., process_audio(filepath)
